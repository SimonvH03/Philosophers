...
echo -e '\e[91m _____ _   _ _                 _                  _____        _
...
if [ "$#" -ne 2 ]; then
    echo -e "Usage: start.sh <Project Folder> <Test Type>"
    echo -e "\tType 0: test philo, and philo_bonus"
    echo -e "\tType 1: test philo only"
    echo -e "\tType 2: test philo_bonus only"
...
echo -e "[+] Test Type: $2\e[0m\n"
echo -e "\e[94m[+] In Case of a failed test, please check ./errors_log file for more information\e[0m\n"

error_log ()
{
    echo -e "[$1-$2]: $3" >> ./errors_log
}
...
test_one ()
{
    ...
    if [ "$output" = "died" ];then
        echo -e "\e[92m[+] Test #1 Succeeded !\e[0m"
    else
...
test_two ()
{
    echo -e "\e[94m[+] Test #2: Executing your program for 180 second, please wait...\e[0m"
    ...
    while [ $i -lt 180 ];do
        ...
        pgrep $1 > /dev/null
        if [ "$?" -ne 0 ];then
            echo -e "\r\e[91m[+] Test #2 Failed\e[0m"
    ...
    if [ $error -eq 0 ];then
        pkill $1
        echo -e "\r\e[92m[+] Test #2 Succeeded\e[0m"
    fi
}

test_three ()
{
    echo -e "\e[94m[+] Test #3: Executing your program for 180 second, please wait...\e[0m"
    ...
    while [ $i -lt 180 ];do
        ...
        pgrep $1 > /dev/null
        if [ "$?" -ne 0 ];then
            echo -e "\r\e[91m[+] Test #3 Failed\e[0m"
    ...
    if [ $error -eq 0 ];then
        pkill $1
        echo -e "\r\e[92m[+] Test #3 Succeeded\e[0m"
    fi
}
...
test_four ()
{
    ...
    if [ "$?" -eq 1 ];then
        ...
        if [ $lines -ge $4 ];then
            echo -e "\t\e[92m[+] Test #4-$5 Succeeded\e[0m"
        else
            echo -e "\t\e[91m[+] Test #4-$5 Failed\e[0m"
...
test_five ()
{
    echo -e "\e[94m[+] Test #5 on progress, please wait...\e[0m"
    i=1
    t=0
    error=0
    while [ $i -le 10 ];do
    ...
    if [ $error -eq 0 ];then
        echo -e "\r\e[92m[+] Test #5 Succeeded\e[0m"
    fi
}
...
if [ "$2" -eq 1 -o "$2" -eq 0 ];then
    ...
    echo -e "\e[94m[+] Test #4 on progress, please wait...\e[0m"
